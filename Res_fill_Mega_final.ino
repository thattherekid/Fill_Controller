  ...
  if(is_pressed(10,210,110,310,px,py)) // Button 1
  {
    ...
    if(switch_flag_2)
    {
      ...
      do{  
        ...
        stopPump = digitalRead(pumpStop);
        resFull = digitalRead(fullSensor);
      //} while(currentGallons < G && digitalRead(fullSensor) == HIGH && digitalRead(pumpStop) == HIGH);
      } while(currentGallons < G && resFull == HIGH && stopPump == HIGH);
      ...
      if(stopPump == LOW){  
        my_lcd.Set_Draw_color(WHITE);
        my_lcd.Fill_Rectangle(1, 140, 479, 190);
        show_string("EMERGENCY STOP", 20, 160, 3, RED, BLACK, 1); 
      }
