...
void fillGallons(){ // Fill G gallons  
  // Swap buttons to Pump Stop
  ...
  my_lcd.Fill_Rectangle(11, 211, 109, 309);
  show_string("PUMP", 25,230,3,WHITE,BLACK,1);
  show_string("STOP", 25,270,3,WHITE,BLACK,1);

  my_lcd.Set_Draw_color(RED);
  my_lcd.Fill_Rectangle(131, 211, 229, 309);
  show_string("PUMP", 145,230,3,WHITE,BLACK,1);
  show_string("STOP", 145,270,3,WHITE,BLACK,1);

  my_lcd.Set_Draw_color(RED);
  my_lcd.Fill_Rectangle(251, 211, 349, 309);
  show_string("PUMP", 265,230,3,WHITE,BLACK,1);
  show_string("STOP", 265,270,3,WHITE,BLACK,1);

  my_lcd.Set_Draw_color(RED);
  my_lcd.Fill_Rectangle(371, 211, 469, 309);
  show_string("PUMP", 385,230,3,WHITE,BLACK,1);
  show_string("STOP", 385,270,3,WHITE,BLACK,1);

  // for testing, display G value for 5 seconds
  //my_lcd.Set_Draw_color(WHITE);
  //my_lcd.Fill_Rectangle(1, 140, 479, 190);
  //show_string(G, 50, 160, 3, GREEN, BLACK, 1);
  //delay(5000);

  if(G < 20000){ // Display which gallon fill is running
    my_lcd.Set_Draw_color(WHITE);
    my_lcd.Fill_Rectangle(1, 120, 479, 140);
    char buf4[3];
    sprintf(buf4, "%01d", AMT);
    show_string(buf4, 10, 120, 3, GREEN, BLACK, 1); // need to fix G to ml
    show_string("GALLON FILL RUNNING", 50, 120, 3, GREEN, BLACK, 1);
  }
  else{
    my_lcd.Set_Draw_color(WHITE);
    my_lcd.Fill_Rectangle(1, 120, 479, 140);
    show_string("FILLING RESERVOIR", 20, 120, 3, GREEN, BLACK, 1);
  }*/
...
void loop() {
  ...
  if((millis() - currentMillis) > 30000){  // update display every 30 seconds with tank volumes
    resLevel1 = map(analogRead(senderGauge1),295,785,0,100); // unsure of values
    getROLevel();
    my_lcd.Set_Draw_color(WHITE);
    my_lcd.Fill_Rectangle(200, 38,  479, 56);
  ...
  if((millis() - displayMillis) > 1000){ // update time every second
    my_lcd.Set_Draw_color(WHITE);  
    my_lcd.Fill_Rectangle(200, 78,  479, 96);
    char buf1[8]; // These 3 lines are causing a reboot after pump stop
